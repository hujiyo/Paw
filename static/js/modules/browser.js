...
export class Browser {
    static init() {
        ...
        this.iframe = $('#browser-frame');
        this.input = $('#browser-url');
        this.goBtn = $('#browser-go');
    ...
    static refresh() {
        ...
        messagesEl.querySelectorAll('.msg').forEach((msgEl, msgIndex) => {
            ...
            msgEl.querySelectorAll('.tool').forEach((toolEl, toolIndex) => {
                const rawResponse = toolEl.dataset.rawResponse;
                const toolName = toolEl.querySelector('.tool__name')?.textContent;
                if (!rawResponse) {
     ...
     * 创建单个URL条目元素
     */
    static createUrlElement(entry, index) {
        ...
        el.addEventListener('click', (e) => {
            // 如果点击的是复制按钮，不触发打开URL
            if (e.target.classList.contains('browser-result__copy-btn')) {
                return;
            }
            this.openUrl(entry);
        });
