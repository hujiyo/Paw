...
export const Settings = {
    ...
    init(sendFn) {
        ...
        // Home 目录浏览按钮
        $('#cfg-home-dir-browse-btn')?.addEventListener('click', async () => {
            if (window.electronAPI) {
                const path = await window.electronAPI.selectFolder();
                if (path) {
                    const input = $('#cfg-home-dir');
                    if (input)
                        input.value = path;
                }
            }
    ...
    selectOption(value, opts) {
        if (!opts.hiddenInputId || !opts.valueDisplayId || !opts.dropdownId)
            return;
            ...
            calibrateBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (result.success && thresholdInput) {
                        ...
                        this.showCalibrateStatus('success', `推荐阈值: ${result.threshold} (质量: ${result.quality})`);
                        // 3秒后隐藏状态
    ...
    setupThemeSelector() {
        ...
        options.forEach(option => {
            option.addEventListener('click', () => {
                const theme = option.dataset.theme || 'yaoye';
    ...
    selectTheme(theme) {
        ...
        // 更新选中状态
        options.forEach(option => {
            option.classList.toggle('theme-option--active', option.dataset.theme === theme);
        });
    },
    async open() {
        if (this.panel)
    ...
    populateForm(config) {
        ...
        const cfgHoney = $('#cfg-honey');
        if (cfgName)
            cfgName.value = identity.name || '';
        if (cfgUsername)
            cfgUsername.value = identity.username || '';
        if (cfgHoney)
            cfgHoney.value = identity.honey || '';
        // API 配置
        const api = config.api || {};
        const cfgApiKey = $('#cfg-api-key');
        const cfgApiUrl = $('#cfg-api-url');
        if (cfgApiKey)
            cfgApiKey.value = api.key || '';
        if (cfgApiUrl)
            cfgApiUrl.value = api.url || '';
        this.selectModel(api.model || '');
        // 终端配置
        const terminal = config.terminal || {};
        const cfgShell = $('#cfg-shell');
        const cfgEncoding = $('#cfg-encoding');
        const cfgBufferSize = $('#cfg-buffer-size');
        if (cfgShell)
            cfgShell.value = terminal.shell || 'powershell';
        if (cfgEncoding)
            cfgEncoding.value = terminal.encoding || 'utf-8';
        if (cfgBufferSize)
            cfgBufferSize.value = String(terminal.buffer_size || 24);
        // Web 配置
        const web = config.web || {};
        const cfgSearchEngine = $('#cfg-search-engine');
        const cfgMaxResults = $('#cfg-max-results');
        const cfgUseJina = $('#cfg-use-jina');
        if (cfgSearchEngine)
            cfgSearchEngine.value = web.search_engine || 'duckduckgo';
        if (cfgMaxResults)
            cfgMaxResults.value = String(web.max_results || 5);
        if (cfgUseJina)
            cfgUseJina.checked = web.use_jina_reader !== false;
        // 系统配置
        const system = config.system || {};
        const cfgHomeDir = $('#cfg-home-dir');
        const cfgChunkSize = $('#cfg-chunk-size');
        if (cfgHomeDir)
            cfgHomeDir.value = system.home_dir || '';
        if (cfgChunkSize)
            cfgChunkSize.value = String(system.chunk_size || 64000);
        // 记忆系统配置
        const memory = config.memory || {};
        const memoryEnabled = memory.enabled || false;
        const cfgMemoryEnabled = $('#cfg-memory-enabled');
        const memoryConfigFields = $('#memory-config-fields');
        if (cfgMemoryEnabled)
            cfgMemoryEnabled.checked = memoryEnabled;
        if (memoryConfigFields)
            memoryConfigFields.style.display = memoryEnabled ? 'block' : 'none';
        // 设置 Embedding URL
        const embeddingUrl = memory.embedding_url || 'http://localhost:11434/api/embeddings';
        const cfgEmbeddingUrl = $('#cfg-embedding-url');
        const cfgEmbeddingUrlPreset = $('#cfg-embedding-url-preset');
        if (cfgEmbeddingUrl)
            cfgEmbeddingUrl.value = embeddingUrl;
        // 检测预设选项
        ...
        const cfgEmbeddingKey = $('#cfg-embedding-key');
        if (cfgEmbeddingKey)
            cfgEmbeddingKey.value = memory.embedding_key || '';
        // 更新 Embedding 下拉
        ...
        const cfgRecallThreshold = $('#cfg-recall-threshold');
        if (cfgRecallThreshold)
            cfgRecallThreshold.value = String(recall.threshold || 0.35);
        // 颜色主题配置 - 根据颜色值匹配主题预设
    ...
    async save() {
        ...
        const cfgEmbeddingUrl = $('#cfg-embedding-url');
        const cfgEmbeddingKey = $('#cfg-embedding-key');
        const cfgEmbeddingModel = $('#cfg-embedding-model');
        const cfgRecallThreshold = $('#cfg-recall-threshold');
