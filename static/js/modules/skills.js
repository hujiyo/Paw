...
export const Skills = {
    ...
    bindEvents() {
        // 工具栏打开/关闭按钮
        const openBtn = $('#skills-market-btn');
        const chatBtn = $('#chat-view-btn');
        if (openBtn) {
        ...
        $$('.skills-market-tab').forEach(tab => {
            tab.addEventListener('click', () => {
                const view = tab.dataset.skillsView;
                this.switchView(view);
            });
        });
        // 搜索输入和按钮
        const searchInput = $('#skills-search-input');
        const categorySelect = $('#skills-category-select');
    ...
    show() {
        const view = $('#skills-market-view');
        const chatView = $('#chat-view');
        const skillsBtn = $('#skills-market-btn');
        const chatBtn = $('#chat-view-btn');
        if (view && chatView) {
            view.classList.add('skills-market-view--active');
            chatView.classList.remove('chat-view--active');
            if (skillsBtn)
                skillsBtn.classList.add('toolbar__btn--active');
            if (chatBtn)
                chatBtn.classList.remove('toolbar__btn--active');
            this.isVisible = true;
            // 打开时加载数据
    ...
    hide() {
        ...
        if (view && chatView) {
            ...
            if (skillsBtn)
                skillsBtn.classList.remove('toolbar__btn--active');
            if (chatBtn)
                chatBtn.classList.add('toolbar__btn--active');
    ...
    switchView(view) {
        ...
        // 切换视图
        const marketContent = $('#skills-market-content');
        const installedContent = $('#skills-installed-content');
    ...
    async searchSkills(query, category) {
    ...
    renderSkillsList(skills) {
        ...
        skills.forEach(skill => {
            ...
            `;
            // 安装按钮事件
            const installBtn = card.querySelector('.skill-card__install-btn');
    ...
    async installSkill(skillId, skillName, repoUrl, button) {
        // 更新按钮状态
    ...
    renderInstalledList(skills) {
        ...
        skills.forEach(skill => {
            ...
            `;
            // 卸载按钮事件
            const uninstallBtn = card.querySelector('.installed-card__uninstall-btn');
            if (uninstallBtn) {
    ...
    async uninstallSkill(skillName, cardElement) {
