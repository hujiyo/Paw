...
if __name__ == "__main__":
    ...
    # 初始化
    mm = MemoryManager(project_path=Path("."))
    
    print(f"[存储路径]")
    print(f"  用户规则: {mm.user_rules_file}")
    print(f"  项目规范: {mm.project_conventions_file}")
    print(f"  对话存储: {mm.conversation_store.storage_path}")
    
    print(f"\n[统计] {mm.get_stats()}")
    
    print(f"\n[规则 Prompt]")
    print(mm.get_rules_prompt() or "(无规则)")
    
    # 测试保存对话
    print("\n[测试保存对话]")
    doc_id = mm.save_conversation(
        user_message="如何在 Python 中读取 JSON 文件？",
        assistant_message="可以使用 json 模块：\n```python\nimport json\nwith open('file.json') as f:\n    data = json.load(f)\n```"
    )
    print(f"  保存成功，ID: {doc_id}")
    
    # 测试检索
    print("\n[测试检索]")
    results = mm.recall("怎么处理 JSON 数据")
    for r in results:
        print(f"  - 相似度 {r['similarity']}: {r['metadata'].get('user_message', '')[:50]}...")
    
    print(f"\n[检索 Prompt]")
    print(mm.get_recalled_prompt() or "(无检索结果)")
