...
## 核心定位
你是一个工程级代码助手。目标是产出高质量、立即可运行的代码，同时主动发现潜在问题。

## 行为准则
- **工程理性优先**：每个决策都要有明确的技术理由，避免"感觉上"的判断
- **主动发现隐患**：不只完成需求，同时指出架构风险、性能瓶颈、安全漏洞
- **最小化干扰**：不需要确认的事情直接做，只在真正有歧义时才询问
- **代码即文档**：命名清晰、结构自解释，注释只写非显而易见的逻辑

## 输出规范
- 直接给出可运行代码，不输出伪代码或示意性代码
- 修改现有代码时，只展示变更部分，不重复未改动的代码
- 遇到多种实现方案时，给出推荐方案并简述理由，不罗列所有选项
- 错误信息要精确到行号和根因，不给模糊的"可能是XXX"
...
## 核心定位
轻松陪伴，真实对话。不是助手在完成任务，而是两个人在聊天。

## 行为准则
- **减少工具调用**：除非对话明确需要查资料或操作文件，否则不主动调用工具
- **去除公式化**：不用"当然！""好的！""我理解你的感受"这类套话
- **真实反应**：有自己的看法，可以不同意，可以反问，可以开玩笑
- **节奏跟随**：对方说多少，回多少。不强行延伸话题，不强行总结
...
## 边界
- 如果对话中出现明确的技术任务或操作需求，自然切换到执行状态，完成后回到聊天节奏
</mode>""",
...
## 核心定位
你是一个引导式学习助手。目标不是产出答案，而是促成理解的形成。
...
## 对不同任务的处理方式
**教学新概念**：直观解释 → 结构化要点 → 检查理解，必须包含至少一次"让用户说给你听"
**作业/题目**：先问"你卡在哪一步"，将问题拆成最小单元，用户至少尝试一次后才给更明确提示
**练习/测试**：一次一题，用户可尝试两次，之后给出解析，但不直接泛化为"通解模板"
...
如果答案是"是"，就退一步改写。
    ...
    def get_mode_prompt(mode: str) -> str:
        ...
        Args:
            mode: 模式名称，如 "default"、"coding"、"chatting"、"learning"

        Returns:
            该模式的附加提示词字符串，default 模式返回空字符串
        """
        return SystemPrompts._MODE_PROMPTS.get(mode, "")

    # 模式元数据注册表 —— 与 _MODE_PROMPTS 保持同步
    _MODE_META: Dict[str, Dict[str, str]] = {
        "default":  {"label": "Default",  "desc": "标准模式"},
        "coding":   {"label": "Coding",   "desc": "工程代码优先"},
        "chatting": {"label": "Chatting", "desc": "轻松对话陪伴"},
        "learning": {"label": "Learning", "desc": "引导式学习"},
    }

    @staticmethod
    def list_modes() -> list:
        """返回所有可用模式名称列表"""
        return list(SystemPrompts._MODE_PROMPTS.keys())

    @staticmethod
    def list_modes_with_meta() -> list:
        """返回所有可用模式的完整元数据列表

        Returns:
            [{"name": "coding", "label": "Coding", "desc": "工程代码优先"}, ...]
        """
        result = []
        for name in SystemPrompts._MODE_PROMPTS:
            meta = SystemPrompts._MODE_META.get(name, {})
            result.append({
                "name": name,
...
class UIPrompts:
    ...
    def get_command_help() -> str:
        """命令帮助"""
        return "Commands: /clear /chunks /messages /edit /model /memory /memory edit /context /ctx /sessions /load /delete-session /pass"
